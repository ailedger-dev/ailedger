...
This is the cross-language settlement point for canonicalization: producers
(agent substrates emitting decision events), the TypeScript SDK/relay, and
verification tooling must serialize byte-identically or every digest and
chain link diverges. The contract is enforced, not assumed: the shared
golden-vector corpus (testdata/jcs-golden-vectors.json, generated from the
production TypeScript `canonicalize` package) pins this implementation in CI,
alongside the SDK and CLI.

The two easy-to-miss divergences handled explicitly:
...
Apache 2.0, stdlib-only, importable by any producer:
...
def _write_object(obj: Mapping[object, object], out: list[str]) -> None:
    ...
    # escapes) and preserves the ordering JS would apply.
    keys = sorted(obj, key=lambda k: k.encode("utf-16-be", "surrogatepass"))  # type: ignore[union-attr]
    out.append("{")
    for i, key in enumerate(keys):
        if i:
            out.append(",")
        _write_string(key, out)  # type: ignore[arg-type]
        out.append(":")
        _write(obj[key], out)
    out.append("}")
...
def _format_float(x: float) -> str:
    ...
    if m is None:  # pragma: no cover - repr of a finite float always matches
        raise ValueError(f"unparseable float repr: {x!r}")
    int_part = m["int"]
    frac = m["frac"] or ""
    exp = int(m["exp"] or 0)
    raw = int_part + frac
