...
export function mountCollective(canvas) {
  ...
  if (!gl) { canvas.dataset.renderState = 'unavailable'; return { setPaused() {}, destroy() {} }; }
  const vertex = `
  precision mediump float;
  attribute vec3 aPosition; attribute vec3 aNormal; attribute float aPath;
  uniform float uTime; uniform float uAspect; uniform vec2 uPointer; uniform float uPoint;
  varying vec3 vNormal; varying vec3 vWorld; varying float vPath; varying float vDepth;
  mat3 rx(float a){float c=cos(a),s=sin(a);return mat3(1.,0.,0.,0.,c,s,0.,-s,c);}
  mat3 ry(float a){float c=cos(a),s=sin(a);return mat3(c,0.,-s,0.,1.,0.,s,0.,c);}
  mat3 rz(float a){float c=cos(a),s=sin(a);return mat3(c,s,0.,-s,c,0.,0.,0.,1.);}
  void main(){
    mat3 r=rz(-.38)*rx(.56+uPointer.y*.12)*ry(.37+sin(uTime*.065)*.13+uPointer.x*.2);
    vec3 p=r*aPosition;
    float d=7.8-p.z;
  ...
  const loc=Object.fromEntries(['uTime','uAspect','uPointer','uMode','uOpacity','uPoint'].map(k=>[k,gl.getUniformLocation(program,k)]));
  const attrs=['aPosition','aNormal','aPath'].map(k=>gl.getAttribLocation(program,k));
  ...
  function sample(t,v,l){
    ...
    const m=Math.hypot(...n)||1;return [...p,...n.map(k=>k/m),t/(Math.PI*2)+l*.19+v*.03];
  }
  ...
  for(let l=0;l<9;l++)for(let i=0;i<160;i++){
    const f=(u)=>{const t=u*TAU*.79-.15;const r=2.65+l*.024;return [r*Math.cos(t),r*.85*Math.sin(t),-.65+Math.sin(t)*.2,0,0,1,u+l*.05];};
    lines.push(...f(i/160),...f((i+1)/160));
  }
  const buffers=[];
  function upload(data){const b=gl.createBuffer();buffers.push(b);gl.bindBuffer(gl.ARRAY_BUFFER,b);gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(data),gl.STATIC_DRAW);return {b,count:data.length/7};}
  ...
  function draw(buffer,mode,opacity,primitive,size=1){
    ...
    attrs.forEach((a,i)=>{gl.enableVertexAttribArray(a);gl.vertexAttribPointer(a,i===2?1:3,gl.FLOAT,false,28,i===0?0:i===1?12:24);});
  ...
  let width=1,height=1,paused=false,visible=true,dead=false,raf=0,last=0,time=0;
  ...
  function render(){
    ...
    gl.uniform1f(loc.uTime,time);gl.uniform1f(loc.uAspect,width/height);gl.uniform2f(loc.uPointer,...pointer);
